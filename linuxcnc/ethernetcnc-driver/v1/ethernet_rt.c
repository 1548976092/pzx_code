...
static int init_net()
{
	...
	//domain指明通信域：PF_UNIX(unix域)，PF_INET(IPv4)，PF_INET6(IPv6)
	//type 通信类型：SOCK_STREAM 是数据流,一般是tcp/ip协议的编程；SOCK_DGRAM分是数据包,是udp协议网络编程
	//protocol指定协议：TCP参数可指定为IPPROTO_TCP,UDP可以用IPPROTO_UDP

	if (sockfd < 0) {
		LL_PRINT("%s: ERROR: can't open socket: %s\n", modname, strerror(errno));
		return -1;
	}
	...
	local_addr.sin_family = AF_INET;//AF_INET	IPv4
	local_addr.sin_port = htons(20202);
	local_addr.sin_addr.s_addr = INADDR_ANY;//INADDR_ANY =0自行分配
	ret = bind(sockfd, (struct sockaddr *) &local_addr, sizeof(struct sockaddr_in));//分配内存地址给socket
	if (ret < 0) {
		LL_PRINT("%s: ERROR: can't connect: %s\n", modname, strerror(errno));
		return -1;
	}
	//设置套接字超时的第三个技术是使用套接字选项 SO_RCVTIMEO 与 SO_SNDTIMEO，
	...
	if (ret < 0) {
		LL_PRINT("%s: ERROR: can't set socket option: %s\n", modname, strerror(errno));
		return -errno;
	}
	...
	//LL_PRINT("%s-> INADDR_ANY：%d board_ip：%s\n", modname, INADDR_ANY,board_ip);
...
static int parse_config()
{
	...
	while(fgets(line, 100, fp) != NULL)
	{
	    ...
	    else if(!strcmp("INPUT", lineptr))
	    {
			...
			//LL_PRINT("ethcnc->inputs[number_of_inputs].name %s\n", ethcnc->inputs[number_of_inputs].name);
